...
run_test() {
    ...
    if [ "$LEAKS_CHECK" == 1 ]; then
        # echo $"valgrind --leak-check=full --show-leak-kinds=all "${command[@]}" $param 2>&1 >/dev/null | grep -ic -e \"LEAK SUMMARY:\" -e \"ERROR SUMMARY: [^0]\" >/dev/null"
