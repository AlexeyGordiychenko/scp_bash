...
run_test() {
    ...
    if [ "$LEAKS_CHECK" == 1 ]; then
        # echo "valgrind --leak-check=full --show-leak-kinds=all "${command[@]}" $param 2> >(grep -ic -e \"LEAK SUMMARY:\" -e \"ERROR SUMMARY: [^0]\" 2>/dev/null 1>&2)"
        valgrind --leak-check=full --show-leak-kinds=all "${command[@]}" $param 2>&1 >/dev/null | grep -ic -e "LEAK SUMMARY:" -e "ERROR SUMMARY: [^0]" >/dev/null
    else
        cmp -s <("${command[@]}" $param 2>/dev/null) <("${sys_command[@]}" $param 2>/dev/null)
    fi
